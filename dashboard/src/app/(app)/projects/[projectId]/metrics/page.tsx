...
const REASON_COLORS: Record<string, string> = {
  TARGETED: "bg-[var(--signal-bg-success-muted)] text-emerald-700 ring-emerald-200",
  ROLLOUT: "bg-blue-100 text-blue-700 ring-blue-200",
  FALLTHROUGH: "bg-[var(--signal-bg-secondary)] text-[var(--signal-fg-secondary)] ring-[var(--signal-border-default)]",
  DISABLED: "bg-amber-100 text-amber-700 ring-amber-200",
  DEFAULT: "bg-[var(--signal-bg-secondary)] text-[var(--signal-fg-secondary)] ring-[var(--signal-border-default)]",
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-6">
        <Card className="p-4 text-center hover:shadow-lg hover:border-[var(--signal-border-emphasis)] sm:p-6">
        ...
        <Card className="p-4 text-center hover:shadow-lg hover:border-[var(--signal-border-emphasis)] sm:p-6">
          ...
          <p className="mt-2 text-3xl font-bold text-[var(--signal-fg-primary)] sm:text-4xl">
            {topFlags.length}
          </p>
        </Card>
      </div>
      ...
      <Card className="hover:shadow-lg hover:border-[var(--signal-border-emphasis)]">
        <CardHeader>
          <h2 className="font-semibold text-[var(--signal-fg-primary)]">Evaluation Reasons</h2>
        ...
        <CardContent>
          {reasonBreakdown.length === 0 ? (
            <p className="text-sm text-[var(--signal-fg-tertiary)] text-center py-4">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3">
              {reasonBreakdown.map(([reason, count]) => (
                <div
                  key={reason}
                  className={`rounded-lg px-3 py-2 ring-1 sm:px-4 sm:py-3 ${REASON_COLORS[reason] || "bg-[var(--signal-bg-secondary)] text-[var(--signal-fg-secondary)] ring-[var(--signal-border-default)]"}`}
                >
      ...
      <Card className="hover:shadow-lg hover:border-[var(--signal-border-emphasis)]">
        <CardHeader>
          <h2 className="font-semibold text-[var(--signal-fg-primary)]">Top Evaluated Flags</h2>
          <p className="mt-0.5 text-xs text-[var(--signal-fg-secondary)]">
        ...
        <div className="divide-y divide-slate-100">
          {topFlags.length === 0 ? (
            <p className="text-sm text-[var(--signal-fg-tertiary)] text-center py-8">
              No evaluations recorded yet.
            </p>
          ) : (
            topFlags.map(([key, count]) => (
              ...
              >
                <span className="w-28 truncate font-mono text-sm font-medium text-[var(--signal-fg-primary)] sm:w-48">
                  {key}
                </span>
                <div className="flex-1 h-5 rounded-full bg-[var(--signal-bg-secondary)] sm:h-6">
                  <div
                    className={`h-full rounded-full bg-[var(--signal-bg-accent-emphasis)] transition-all w-[${Math.max(2, (count / maxFlagCount) * 100)}%]`}
                  />
                </div>
                <span className="w-16 text-right text-sm font-semibold text-[var(--signal-fg-primary)] sm:w-20">
                  {count.toLocaleString()}
                </span>
              </div>
            ))
