...
export function FlagConfigurationSlideOver({
  ...
  const [activeTab, setActiveTab] = useState<
    "targeting" | "variations" | "governance"
  >("targeting");
      ...
      <div className="relative w-full max-w-3xl bg-white h-full shadow-2xl flex flex-col border-l border-stone-200 animate-in slide-in-from-right duration-300">
        ...
        <header className="px-8 py-6 border-b border-stone-100 bg-stone-50 flex items-start justify-between shrink-0">
          <div>
            ...
            <div className="flex items-center space-x-2 text-stone-500 font-mono text-sm">
              <span>key:</span>
              <span className="bg-stone-200 px-1.5 py-0.5 rounded text-stone-700 select-all">
                {flagKey}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-6">
            ...
            <button
              onClick={onClose}
              className="text-stone-400 hover:text-stone-800 bg-white hover:bg-stone-100 border border-stone-200 p-2 rounded-full transition-colors shadow-sm"
            >
              <svg
                className="w-5 h-5"
                fill="none"
            ...
            <div className="flex space-x-3">
              ...
              <div>
                ...
                <p className="text-amber-700 text-sm mt-1">
                  This flag has served 100% "True" for 45 days in Production. It
                  is safe to remove.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {rules.map((rule, index) => (
                  <div
                    key={rule.id}
                    className="bg-white border border-stone-200 rounded-xl shadow-sm overflow-hidden transition-all hover:border-stone-300"
                  >
                ...
                <div className="bg-stone-100 border border-stone-200 rounded-xl overflow-hidden opacity-80">
                  <div className="px-5 py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="font-bold text-stone-500 bg-stone-200 px-3 py-1 rounded">
                        False
                      </span>
                    </div>
                  </div>
        ...
        <footer className="px-8 py-5 border-t border-stone-200 bg-white flex items-center justify-between shrink-0">
          ...
          <div className="flex space-x-3">
            <button
              onClick={onClose}
              className="px-5 py-2.5 rounded-md font-semibold text-stone-600 hover:bg-stone-100 transition-colors"
            >
              Cancel
            </button>
