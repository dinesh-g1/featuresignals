...
| [[CLICKHOUSE_SCHEMA.md]] | `current` | **ClickHouse Schema — Evaluation Events Analytics Store v1.0.0.** Comprehensive design document covering: why ClickHouse over PostgreSQL for OLAP, schema design (MergeTree, monthly partitioning, bloom filters, materialized views), data type rationale, TTL policies per tier, ingestion pipeline architecture (SDK→Go Server→NATS→ClickHouse), query patterns for dashboard/billing/operations, PostgreSQL migration plan (dual-write→verify→cutover), resource estimates (4 TB at 10K events/s), Go server connection configuration, security (TLS, access control, GDPR pseudonymization), and monitoring/alerting. Schema at `server/internal/migrate/clickhouse/001_eval_events.sql`. Closes P0 item #4 from PRE_IMPLEMENTATION_GAP_ANALYSIS. | high |
...
| [[GITHUB_APP_SPECIFICATION.md]] | `current` | **GitHub App Specification — Code2Flag Integration.** Formal, implementable specification: 10 sections covering OAuth scopes (3 required: `repo`, `admin:repo_hooks`, `user`), webhook events (push, pull_request.opened, pull_request.closed), 22 API endpoints cataloged, rate limit strategy (5,000 req/hour, exponential backoff with jitter), token lifecycle (AES-256-GCM encryption, JANITOR_ENCRYPTION_KEY), 10-step connection flow, 6 connection states, security (encryption, logging safety, cross-org isolation, CSRF), 10-error-scenario contract, GitLab & Bitbucket parity comparison tables. 11 concrete requirement IDs (FS-S0-INT-001-*). Closes P0 item #10 from PRE_IMPLEMENTATION_GAP_ANALYSIS. | high |
| [[IMPORT_TOOL_SPECIFICATION.md]] | `current` | **Import Tool Specification — Competitor Migration Engine.** Formal specification for the import tool covering: 3 supported source platforms (LaunchDarkly, Flagsmith, Unleash) with complete entity-by-entity data mappings, operator translation tables, 6-tier mismatch handling, 9-step import process flow, parity test specification (16 flag categories, 100+ contexts/flag, 99.5% pass threshold), bulk import UX (5 dashboard states), SDK drop-in replacement guide structure (8 languages × 3 competitors), rate limiting with exponential backoff, and security (AES-256-GCM encryption, key redaction). References FS-S1-MIG-001 and FS-S1-MIG-002. Closes P0 item #13 from PRE_IMPLEMENTATION_GAP_ANALYSIS (formal specification gap). | high |
...
| [[NATS_TOPIC_SPECIFICATION.md]] | `current` | **NATS Topic & Message Specification v1.0.0.** Formal specification for all NATS-based inter-service communication. 274 lines, 7 sections: subject hierarchy with 13 subjects across 7 domains (eval, agent, billing, audit, cache, webhook, health), JetStream stream configuration (5 streams with retention/age/size/replicas), consumer groups (5 groups with concurrency and acknowledgment), message envelope rules and schema, OpenTelemetry trace propagation via NATS headers, deployment configuration (docker-compose, env vars, production notes), operational procedures (graceful shutdown, reconnection, dead-letter queue, message replay), and monitoring/alerting (6 metrics, 4 alerts). Closes P0 items #9 and #20 from PRE_IMPLEMENTATION_GAP_ANALYSIS. | high |
| [[THREAT_MODEL.md]] | `current` | **Comprehensive Threat Model v1.0.0.** 47 threats across 6 STRIDE categories + 4 LLM-specific categories. Covers: asset inventory (6 categories), threat actors (6 profiles), STRIDE analysis (24 threats with severity/likelihood/mitigations/gaps), LLM-specific threats (prompt injection, evaluation poisoning, agent impersonation, model extraction), cross-org data leakage deep-dive, infrastructure security controls inventory, incident response procedures with P0-P3 SLAs, 10-item residual risk register, security testing program (CI + manual), and SOC2/ISO27001/GDPR/DORA compliance mapping. 27 actionable recommendations across P0/P1/P2 priorities. 10 PRS requirement IDs (FS-SEC-TM-001 through FS-SEC-TM-010). Closes P0 item #7 from PRE_IMPLEMENTATION_GAP_ANALYSIS. | high |
| [[INTERNAL_AGENT_PROTOCOL_SPECIFICATION.md]] | `current` | **Internal Agent Protocol (IAP) Specification v1.0.0.** Protocol-agnostic message format for all agent communication. 343 lines, 10 sections: design philosophy with protocol agnosticism invariant, AgentMessage envelope, 21 message types across 6 categories (task, discovery, collaboration, human interaction, governance, audit), 7 typed payload schemas, transport adapter interface (NATS/in-memory/MCP/WebSocket), message lifecycle (send/receive/request-reply), serialization rules with forward-compatibility, operational characteristics (TTL, delivery, idempotency, DLQ), migration & versioning, security. Closes P0 item #16 from PRE_IMPLEMENTATION_GAP_ANALYSIS. | high |
