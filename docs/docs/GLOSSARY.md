...
| **Segment** | A reusable audience definition based on entity attributes. Segments can be referenced across multiple flags' targeting rules. | Example: "Beta Testers", "Enterprise Customers", "Internal Team". |
| **Rule** | A targeting condition within a flag's environment state. Rules specify which segment or attribute conditions determine the flag's value for matching entities. | Use "targeting rule" in full the first time on a page, then "rule" for brevity. |
| **Rollout** | Progressive percentage-based exposure of a flag to entities, using consistent hashing for deterministic assignment. | "10% rollout" means 10% of entities see the flag enabled. Always specify the percentage. |
...
| **Evaluation** | The process of determining a flag's value for a specific entity in a specific environment. Happens in the SDK or via the Evaluate API. | "Flag evaluation" is the full term. "Evaluation" alone is acceptable after first mention. |
| **Entity** | The subject being evaluated — typically a user, device, session, or service. Defined by a `key` (unique identifier) and `attributes` (properties for targeting). | Use "entity" in technical docs and API reference. In product UI, use "user" when the context is clearly about end users. |
