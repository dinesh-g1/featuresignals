...
export const metadata: Metadata = {
  title: "Feature Flag a Checkout Flow",
  description:
    "Tutorial: use feature flags to safely release a new checkout flow with gradual rollout.",
};
...
export default function FeatureFlagCheckoutPage() {
  ...
  -d '{"key": "new-checkout", "name": "New Checkout Flow", "type": "boolean", "defaultValue": false}'`}
      ...
      <CodeBlock
        language="javascript"
        code={`const { FeatureSignals } = require('@featuresignals/node');
...
await client.initialize();`}
  ...
  return handleLegacyCheckout(req, res);
      ...
      <CodeBlock
        language="javascript"
        code={`// After cleanup — no more flag check
app.post('/checkout', async (req, res) => {
  return handleNewCheckout(req, res);
      ...
      <ul className="space-y-2">
        {[
          { label: "A/B Testing in React", href: "/docs/tutorials/ab-testing-react" },
