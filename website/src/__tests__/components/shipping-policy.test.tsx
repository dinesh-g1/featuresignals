...
describe("ShippingPolicyPage", () => {
  it("renders without crashing", async () => {
    const { default: ShippingPolicyPage } =
      await import("@/app/shipping-policy/page");
  ...
  it("renders the digital delivery section", async () => {
    const { default: ShippingPolicyPage } =
      await import("@/app/shipping-policy/page");
    render(<ShippingPolicyPage />);
    expect(screen.getByText("1. Digital Delivery")).toBeInTheDocument();
  });
  ...
  it("renders the SaaS access section", async () => {
    const { default: ShippingPolicyPage } =
      await import("@/app/shipping-policy/page");
    render(<ShippingPolicyPage />);
    expect(screen.getByText("3. SaaS Access")).toBeInTheDocument();
  });
  ...
  it("renders the shipping costs section", async () => {
    const { default: ShippingPolicyPage } =
      await import("@/app/shipping-policy/page");
    render(<ShippingPolicyPage />);
    expect(screen.getByText("5. Shipping Costs")).toBeInTheDocument();
  });
  ...
  it("renders the delivery timelines section", async () => {
    const { default: ShippingPolicyPage } =
      await import("@/app/shipping-policy/page");
    render(<ShippingPolicyPage />);
    expect(screen.getByText("6. Delivery Timelines")).toBeInTheDocument();
  });
  ...
  it("renders the international delivery section", async () => {
    const { default: ShippingPolicyPage } =
      await import("@/app/shipping-policy/page");
    render(<ShippingPolicyPage />);
    expect(screen.getByText("7. International Delivery")).toBeInTheDocument();
  });
  ...
  it("renders the returns & exchanges section", async () => {
    const { default: ShippingPolicyPage } =
      await import("@/app/shipping-policy/page");
    render(<ShippingPolicyPage />);
    expect(screen.getByText("9. Returns & Exchanges")).toBeInTheDocument();
  });
  ...
  it("renders the contact section with email link", async () => {
    ...
    const emailLinks = screen.getAllByText("support@featuresignals.com");
  ...
  it("mentions cloud saas and self-hosted delivery", async () => {
    ...
    expect(
      screen.getAllByText(/open-source software/i).length,
