...
export default function TenantDetailPage() {
  ...
  const handleDeprovision = useCallback(async () => {
    ...
    try {
      ...
      toast.success(
        ...
        `"${tenant.name}" has been deprovisioned.`,
      );
      deprovisionDialog.closeDialog();
      router.push("/tenants");
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Failed to deprovision tenant";
  ...
  const handleMigrate = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to migrate tenant";
      toast.error("Migration failed", message);
    }
  }, [tenant, migrateTargetCell, updateTenantMutation, toast, refetch]);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        <ErrorState
          ...
          message={
            error.message ?? "Unable to fetch tenant details. Please try again."
          }
          onRetry={() => refetch()}
        />
      </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <Badge
                variant={tierBadgeVariant[tenant.tier] ?? "default"}
                size="sm"
              >
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                {tenant.apiKeys.length} key
                {tenant.apiKeys.length !== 1 ? "s" : ""} configured
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent>
            ...
            {!tenant.currentBill ? (
              ...
              <div className="flex flex-col items-center justify-center py-4">
                <p className="text-sm text-text-muted mb-1">
                  Amount so far this period
                </p>
                <p className="text-3xl font-bold tabular-nums text-text-primary">
                  {formatCurrency(
                    tenant.currentBill.amount,
                    tenant.currentBill.currency,
                  )}
                </p>
              </div>
            )}
        ...
        <div className="space-y-4">
          <div className="rounded-lg bg-accent-warning/5 border border-accent-warning/20 p-3">
            <p className="text-xs text-accent-warning">
              <strong>Note:</strong> Migrating a tenant to a different cell may
              cause a brief interruption in service while the transition
              completes.
            </p>
          </div>
