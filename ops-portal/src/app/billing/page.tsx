...
import { Table, createColumnHelper, type ColumnDef } from '@/components/ui/table';
...
const INVOICE_STATUS_MAP: Record<string, 'success' | 'warning' | 'danger' | 'info' | 'default'> = {
  paid: 'success',
  pending: 'info',
  past_due: 'warning',
  failed: 'danger',
...
function buildInvoiceColumns(
  ...
  return [
        ...
        return (
          <span
            className={cn(
              ...
              isOverdue ? 'text-accent-danger font-medium' : 'text-text-secondary',
            )}
          >
    ...
    invoiceColumnHelper.accessor('id', {
      ...
      cell: (info) => {
        const invoice = info.row.original;
        const needsRetry = invoice.status === 'failed' || invoice.status === 'past_due';
        ...
        <div className="flex flex-col items-center justify-center min-h-[40vh]">
          <ErrorState
            ...
            message={
              ...
              ?? (invoicesError instanceof Error ? invoicesError.message : undefined)
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <StatCard
              ...
              value={mrrData ? formatCurrency(mrrData.currentMRR, mrrData.currency) : '€0.00'}
              sublabel={mrrData ? `Based on active subscriptions` : undefined}
              trend={mrrData?.currentMRR && mrrData.currentMRR > 0 ? 'up' : 'neutral'}
            ...
            <StatCard
              ...
              value={mrrData ? formatCurrency(mrrData.avgRevenuePerCustomer, mrrData.currency) : '€0.00'}
            ...
            <StatCard
              ...
              trend={mrrData?.churnRate && mrrData.churnRate > 5 ? 'down' : mrrData?.churnRate && mrrData.churnRate > 0 ? 'neutral' : 'up'}
              trendLabel={mrrData?.churnRate && mrrData.churnRate > 0 ? `${mrrData.churnRate.toFixed(1)}% churn` : 'No churn'}
              loading={mrrLoading && !mrrData}
            />
          </div>
