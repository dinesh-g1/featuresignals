...
for slug in "${SLUGS[@]}"; do
  ...
  if [ -f "$HTML_FILE" ]; then
    ...
    TITLE=$(grep -oP '<title>\K[^<]+' "$HTML_FILE" | head -1 | sed 's/ | Documentation//' | sed 's/ | FeatureSignals//')

    # Extract description from meta tag
    DESCRIPTION=$(grep -oP '<meta[^>]*name="description"[^>]*content="\K[^"]+' "$HTML_FILE" | head -1)
    ...
    cat > "$JSON_FILE" << JSONEOF
{
  ...
  "toc": [],
  "contentHash": "$(echo -n "$slug$CONTENT" | md5 -q 2>/dev/null || echo -n "$slug$CONTENT" | md5sum | cut -d' ' -f1)"
}
JSONEOF
